...
app = Flask(__name__)
app.config['SECRET_KEY'] = 'adora_uxbridge_2025_secure'
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///C:/Users/Desktop/PycharmProjects/FlaskProject4/adora.db'
...
app.config['MAIL_USERNAME'] = 'abbasbooazar430@gmail.com'
app.config['MAIL_PASSWORD'] = 'hbgs rhuk oxeo arxl'
app.config['MAIL_DEFAULT_SENDER'] = 'abbasbooazar430@gmail.com'
...
categories = ['kitchen', 'bathroom', 'flooring', 'wardrobe', 'plumbing', 'carpentry_tiling', '3d_design']
...
def add_service():
    if request.method == 'POST':
        name = request.form.get('name').strip().title()  # Normalize to title case
...
def add_testimonial():
    if request.method == 'POST':
        ...
        if client_name and content and rating:
            new_testimonial = Testimonial(client_name=client_name, content=content, location=location, rating=int(rating))
        ...
        for file in files:
            if file and file.filename.lower().endswith(('.png', '.jpg', '.jpeg')):
                ...
                if os.path.exists(file_path):
                    # Improved service lookup: match category to service name case-insensitively
                    service = Service.query.filter(Service.name.ilike(category.replace('_', ' ').title())).first()
                    app.logger.info(f"Upload: Category={category}, Service={service.name if service else 'None'}, Filename={filename}")
...
def contact(service_id):
    ...
    if request.method == 'POST':
        ...
        # Process selected gallery images
        saved_gallery_files = []
...
def init_db():
    with app.app_context():
        ...
        if not Service.query.first():
            services_data = [
                {'name': 'Kitchen Renovations', 'description': 'Bespoke kitchen installations and repairs in Uxbridge. Cabinets, countertops, and flatpack assembly.'},
                {'name': 'Bathroom Installations', 'description': 'Complete bathroom renovations and repairs in Pinner. Tiling, fixtures, and plumbing.'},
                {'name': 'Flooring Solutions', 'description': 'Durable flooring installations in Ruislip. Hardwood, tile, or laminate.'},
                {'name': 'Wardrobe Installations', 'description': 'Custom wardrobes for Ickenham homes. Built-in storage solutions.'},
                {'name': 'Plumbing Repairs', 'description': 'Expert plumbing services in Ealing. Leak fixes, tap installations, and more.'},
                {'name': 'Carpentry & Tiling', 'description': 'Precision carpentry and tiling in Acton. Jet washing for clean finishes.'},
                {'name': '3D Design Services', 'description': 'Advanced 3D modeling for Richmond projects. Visualize your renovation.'}
            ]
            for data in services_data:
                db.session.add(Service(**data))
        # Sync categories with services, preserving existing categories
        global categories
        ...
        if not Testimonial.query.first():
            testimonials_data = [
                {'client_name': 'Sarah L.', 'content': 'Adora transformed our Uxbridge kitchen with precision.', 'location': 'Uxbridge'},
                {'client_name': 'James P.', 'content': 'Professional bathroom renovation in Pinner. Highly recommended.', 'location': 'Pinner'},
                {'client_name': 'Emma R.', 'content': 'Bespoke wardrobe in Ickenham was perfect.', 'location': 'Ickenham'}
            ]
